...
export class CS210View extends StudentView {
    ...
    public renderPage(opts: {}) {
        Log.info('CS210View::renderPage() - start; options: ' + opts);
        const that = this;
        ...
        super.render().then(function() {
            ...
            Log.info('CS210View::renderPage(..) - prep & render took: ' + UI.took(start));
            UI.hideModal();
        }).catch(function(err) {
            Log.error('CS210View::renderPage() - ERROR: ' + err);
            UI.hideModal();
        });
    }
    ...
    private async renderStudentPage(): Promise<void> {
        UI.showModal('Fetching Data');
        try {
            Log.info('CS210View::renderStudentPage(..) - start');
            ...
            Log.info('CS210View::renderStudentPage(..) - done');
        } catch (err) {
    ...
    private async fetchTeamData(): Promise<TeamTransport[]> {
        try {
            ...
            Log.error('CS210View::fetchTeamData(..) - ERROR: ' + err.message);
            this.teams = [];
            return [];
        }
    }
    ...
    private async renderTeams(teams: TeamTransport[]): Promise<void> {
        Log.trace('CS210View::renderTeams(..) - start');
        const that = this;
        ...
        if (projectTeam === null) {
            ...
            button.onclick = function(evt: any) {
                Log.info('CS210View::renderTeams(..)::createTeam::onClick');
                that.formTeam().then(function(team) {
                    Log.info('CS210View::renderTeams(..)::createTeam::onClick::then - team created');
                    that.teams.push(team);
                    if (team !== null) {
                        that.renderPage({}); // simulating refresh
                    }
                }).catch(function(err) {
                    Log.info('CS210View::renderTeams(..)::createTeam::onClick::catch - ERROR: ' + err);
                });
            };
    ...
    private async formTeam(): Promise<TeamTransport> {
        Log.info("CS210View::formTeam() - start");
        ...
        Log.info("CS210View::formTeam() - URL: " + url + "; payload: " + JSON.stringify(payload));
        const response = await fetch(url, options);

        Log.info("CS210View::formTeam() - responded");

        const body = await response.json() as Payload;

        Log.info("CS210View::formTeam() - response: " + JSON.stringify(body));
        ...
        if (typeof body.success !== 'undefined') {
            ...
            Log.error("CS210View::formTeam() - else ERROR: " + JSON.stringify(body));
        }
    }
